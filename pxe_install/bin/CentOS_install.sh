...
if [ ! -f $curpath/iso/CentOS*.iso ];then
	echoError "ERROR: check $curpath/iso not exists"
	exit
fi
mount $curpath/iso/CentOS*.iso $mount_dir1
...
function http_service_config () {
	...
	mount_dir=/var/www/html/CentOS
	/usr/bin/mkdir -p $mount_dir
	# openEuler
	mount $curpath/iso/CentOS*.iso /$mount_dir
	if mountpoint -q $mount_dir
	then
		/usr/bin/cp $mount_dir/EFI/BOOT/{BOOTAA64.EFI,grubaa64.efi} /var/lib/tftpboot/uefi/
		cd $curpath/config/
		if [ -f CentOS_grub.cfg ];then
			cp CentOS_grub.cfg /var/lib/tftpboot/uefi/grub.cfg
			...
			/usr/bin/sed -i "2a install \nurl --url=http://${net_next}/CentOS" /var/www/html/ks.cfg
